...
import { AUTH_ENDPOINT, GITHUB_URL } from "@/utils/constants";
import { Loader2 } from "lucide-react";
import Link from "next/link";
import { buttonVariants } from "../ui/button";

export default function Navbar() {
  const { user, isLoading } = useSession({ redirectOnAuthFailure: false });
            ...
            <div className="flex items-center gap-4">
              <Link href={GITHUB_URL}>
                <svg
                  ...
                  height="1em"
                  fill="currentColor"
                  className="h-6 w-6 text-foreground opacity-100 duration-300 hover:opacity-50"
                >
